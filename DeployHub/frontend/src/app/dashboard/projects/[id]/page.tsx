...
import {
  ArrowLeft, GitBranch, Upload, Rocket, ExternalLink, RefreshCw,
  Pencil, Save, X, FolderOpen, Server, HardDrive, KeyRound, Globe
} from 'lucide-react'
...
export default function ProjectDetailPage() {
  ...
  async function loadData() {
    try {
      ...
      const [p, d, envData] = await Promise.all([
        api.getProject(id, t),
        api.getDeployments({ projectId: id, limit: 20 }, t),
        api.getProjectEnv(id, t, false).catch(() => ({ count: 0 })),
      ])
      setProject(p); setDeployments(d)
      setEnvCount(envData.count ?? 0)
      setEditForm({
        name: p.name, repoUrl: p.repoUrl || '', framework: p.framework || '',
        buildCommand: p.buildCommand || '', outputDir: p.outputDir || '',
        nodeVersion: p.nodeVersion || '20', projectSubDir: p.projectSubDir || '',
        isBackend: p.isBackend || false,
      })
  ...
  async function saveSettings() {
    ...
    try {
      const t = await getToken(); if (!t) return
      await api.updateProject(id, editForm, t)
    ...
    <div className="p-8 fade-in">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl" style={{ background: 'var(--surface)' }}>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              {project.repoUrl && (
                <a href={project.repoUrl} target="_blank" rel="noreferrer" className="text-sm flex items-center gap-1" style={{ color: 'var(--accent)' }}>
                  <GitBranch className="w-3 h-3" />{project.repoUrl.replace('https://github.com/', '')}
                </a>
              )}
              {project.projectSubDir && (
                <span className="text-sm flex items-center gap-1" style={{ color: 'var(--yellow)' }}>
                  <FolderOpen className="w-3 h-3" />{project.projectSubDir}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="flex gap-2">
          ...
          {/* Env vars button */}
          <Link
            href={`/dashboard/projects/${id}/env`}
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm border transition-colors relative"
            style={{ borderColor: 'rgba(79,142,255,0.4)', color: 'var(--accent)' }}
          >
            <KeyRound className="w-3.5 h-3.5" /> Env Vars
            {envCount > 0 && (
              <span className="ml-0.5 px-1.5 py-0.5 rounded-full text-xs font-bold" style={{ background: 'var(--accent)', color: '#fff' }}>
                {envCount}
              </span>
            )}
          </Link>
          <button onClick={() => setEditing(!editing)} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm border transition-colors" style={{ borderColor: 'var(--border)', color: 'var(--text2)' }}>
      ...
      {editing && (
        <div className="rounded-xl border p-5 mb-6 slide-in" style={{ background: 'var(--surface)', borderColor: 'var(--border2)' }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs mb-1" style={{ color: 'var(--text2)' }}>Name</label>
              <input className={inp} style={ist} value={editForm.name} onChange={e => setEditForm((f: any) => ({ ...f, name: e.target.value }))} /></div>
            <div><label className="block text-xs mb-1" style={{ color: 'var(--text2)' }}>Repo URL</label>
              <input className={inp} style={ist} value={editForm.repoUrl} onChange={e => setEditForm((f: any) => ({ ...f, repoUrl: e.target.value }))} /></div>
            <div>
              <label className="block text-xs mb-1 flex items-center gap-2" style={{ color: 'var(--text2)' }}>
                Project Subdirectory
                <span className="px-1.5 py-0.5 rounded text-xs" style={{ background: 'rgba(79,142,255,0.15)', color: 'var(--accent)' }}>nested repo fix</span>
              </label>
            ...
            <div><label className="block text-xs mb-1" style={{ color: 'var(--text2)' }}>Build Command</label>
              <input className={inp} style={ist} placeholder="npm run build" value={editForm.buildCommand} onChange={e => setEditForm((f: any) => ({ ...f, buildCommand: e.target.value }))} /></div>
            <div><label className="block text-xs mb-1" style={{ color: 'var(--text2)' }}>Output Dir</label>
              <input className={inp} style={ist} placeholder="dist" value={editForm.outputDir} onChange={e => setEditForm((f: any) => ({ ...f, outputDir: e.target.value }))} /></div>
          </div>
          <div
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs mt-0.5" style={{ color: 'var(--text3)' }}>
                  {editForm.isBackend
                    ? 'Deploys Docker container to EC2. Port auto-allocated per project.'
          ...
          <div className="flex gap-3 mt-4">
            <button onClick={saveSettings} disabled={saving} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all hover:opacity-90 disabled:opacity-50" style={{ background: 'var(--accent)', color: '#fff' }}>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
            <div className="px-5 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
              <h3 className="font-semibold text-sm">New Deployment</h3>
            </div>
            <div className="p-5">
              ...
              {tab === 'github' ? (
                <div className="space-y-3">
                  <div><label className="block text-xs mb-1" style={{ color: 'var(--text3)' }}>Repository</label>
                    <div className="px-3 py-2 rounded-lg text-xs truncate" style={{ background: 'var(--surface2)', color: 'var(--text3)' }}>{project.repoUrl || 'No repo configured'}</div></div>
                  {project.projectSubDir && <div><label className="block text-xs mb-1" style={{ color: 'var(--text3)' }}>Project path</label>
                    <div className="px-3 py-2 rounded-lg text-xs font-mono" style={{ background: 'var(--surface2)', color: 'var(--yellow)' }}>📂 {project.projectSubDir}</div></div>}
                  <div><label className="block text-xs mb-1" style={{ color: 'var(--text3)' }}>Branch</label>
                    <input className={inp} style={ist} value={ghBranch} onChange={e => setGhBranch(e.target.value)} placeholder="main" /></div>
                  {deployError && <p className="text-xs" style={{ color: 'var(--red)' }}>{deployError}</p>}
                  <button onClick={deployGH} disabled={deploying || !project.repoUrl}
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all hover:opacity-90 disabled:opacity-40"
                ...
                <div className="space-y-3">
                  <div className="border-2 border-dashed rounded-xl p-6 text-center" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <label htmlFor="zip-input">
                      <span className="px-4 py-2 rounded-lg text-sm border cursor-pointer inline-block transition-colors" style={{ borderColor: 'var(--border2)', color: 'var(--text2)' }}>
                        {zipDeploying ? 'Deploying...' : 'Choose ZIP'}
                      </span>
                    </label>
                  </div>
                  {deployError && <p className="text-xs" style={{ color: 'var(--red)' }}>{deployError}</p>}
          ...
          <div className="rounded-xl border" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
            ...
            <div className="p-5 space-y-3">
              {[
                ['Framework', FRAMEWORK_LABELS[project.framework || ''] || 'Auto-detect'],
                ['Build', project.buildCommand || 'Auto-detect'],
                ['Output', project.outputDir || 'Auto-detect'],
                ['Node', `Node ${project.nodeVersion || '20'}`],
                ['Subdirectory', project.projectSubDir || 'Auto-detect'],
              ].map(([l, v]) => (
                <div key={l} className="flex justify-between text-xs">
                  <span style={{ color: 'var(--text3)' }}>{l}</span>
                  <span className="font-mono" style={{ color: 'var(--text2)' }}>{v}</span>
                </div>
              ))}
              {/* Env vars quick link */}
              <div className="border-t pt-3 mt-1" style={{ borderColor: 'var(--border)' }}>
                <Link href={`/dashboard/projects/${id}/env`} className="flex items-center justify-between text-xs group">
                  <span className="flex items-center gap-1.5" style={{ color: 'var(--text3)' }}>
                    <KeyRound className="w-3 h-3" /> Env Variables
                  </span>
                  <span className="font-mono group-hover:text-white transition-colors" style={{ color: 'var(--accent)' }}>
                    {envCount > 0 ? `${envCount} set →` : 'Add →'}
                  </span>
                </Link>
              </div>
            </div>
        ...
        <div className="col-span-2">
          <div className="rounded-xl border" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
            ...
            {deployments.length === 0
              ? <div className="p-12 text-center text-sm" style={{ color: 'var(--text3)' }}>No deployments yet</div>
              : (
                    ...
                    <Link href={`/dashboard/deployments/${d.id}`} key={d.id}>
                      <div className="flex items-center justify-between px-5 py-4 hover:bg-white/3 transition-colors cursor-pointer">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                              ...
                              {d.detectionMethod && (
                                <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${DETECTION_METHOD_COLORS[d.detectionMethod] || ''}`}>
                                  {DETECTION_METHOD_LABELS[d.detectionMethod] || d.detectionMethod}
                                </span>
                              )}
                              <span className={`text-xs px-1.5 py-0.5 rounded flex items-center gap-1 ${d.isBackend ? 'text-orange-400 bg-orange-400/10' : 'text-cyan-400 bg-cyan-400/10'}`}>
                                {d.isBackend
                                  ? <><Server className="w-2.5 h-2.5" />EC2{d.hostPort ? `:${d.hostPort}` : ''}</>
                                  : <><HardDrive className="w-2.5 h-2.5" />S3</>}
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          {d.previewUrl && (
                            <a href={d.previewUrl} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ color: 'var(--text3)' }}>
